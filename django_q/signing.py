...
class PickleSerializer:
    ...
    def loads(data) -> any:
        ...
        if isinstance(data, dict):
            if isinstance(args := data.get('args', None), tuple):
                data['args'] = tuple(PickleSerializer._load_model_instance(v) for v in args)
            if isinstance(kw := data.get('kwargs', None), dict):
                data['kwargs'] = {k: PickleSerializer._load_model_instance(v) for k, v in kw.items()}
        return data
